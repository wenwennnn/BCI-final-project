...
class EEGCNN(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(1, 32, 3, padding=1),  # (B, 32, 14, 5)
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.Conv2d(32, 64, 3, padding=1),  # (B, 64, 14, 5)
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=(2, 1)),  # (B, 64, 7, 5)
            nn.Conv2d(64, 128, 3, padding=1),  # (B, 128, 7, 5)
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.AdaptiveAvgPool2d((4, 2))  # (B, 128, 4, 2)
        )
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(64, 10)  # 2 outputs x 5 classes
        )
...
plt.tight_layout()
plt.savefig("cnn_training_from_npz_result.jpg", dpi=300)
plt.show()
